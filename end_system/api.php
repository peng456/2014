...
//发送header声明页面编码
header("CONTENT-TYPE:application/json; CHARSET=".END_CHARSET);

//根据url中的p变量来确定对应的controller
$_page = ($_GET['p'])?$_GET['p']:'index';
$_page = preg_replace('/[^a-zA-Z0-9_.]/','',$_page);
$_version = ($_GET['v']) ? $_GET['v'] : '1'; # API 版本号
$_version = preg_replace('/[^0-9]/','',$_version);
...
//load the main controller (API file)

$_c_filename = END_API_DIR.$_version."/".$_controller; ## add version control
file_exists($_c_filename) && include($_c_filename);

## modified by liudan: API call do not need view
echo $_c_filename . '</br>';
die("This message should not appear!!! api call error");
//timer stop
list($_usec, $_sec) = explode(' ', microtime()); 
