...
$user_data = model('vanet_user')->get_one(array('username'=>$username)) ;
if ($user_data)
{
...
$time = time() ;
$checkcode_data = $db->get_all("SELECT * FROM end_vanet_user_checkcode WHERE phone = $data[phone] and ( status = 'valid' and expires_in > $time ) " ) ;
if (!$checkcode_data)
{
...
if (!model('vanet_user_checkcode')->update($checkcode_data[0]['checkcode_id'],array('status'=>'invalid')) )
{
	die_json_msg('database error: update checkcode log error',10003) ;
}


$item = model('vanet_user')->add(array('username'=>$username, 'password'=>$password , 'phone'=>$username));
if (!$item)
{
...
$db->query("update end_vanet_token set status='invalid' where token_type='user' and owner_id=$item and status='valid'");
while (1)
{
