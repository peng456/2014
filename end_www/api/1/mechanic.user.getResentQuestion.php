...
if($data['type'] == 1){
    $question_sql = "select question.* from  end_mechanic_question as question inner join end_mechanic_chat_group as chat_group using(q_id) where question.driver_user_id = {$item['owner_id']} and (question.type = 2 or question.type = 3) order by chat_group.q_end_time desc limit {$data['from']},10";
