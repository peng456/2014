...
if (!isset($data['access_token']) ||(!isset($data['driver_id'])&&!isset($data['huanxin_id'])) )
{
...
elseif(isset($data['huanxin_id'])){
    ...
    $driver_item = model('mechanic_user')->get_one(array('huanxin_id'=>$data['huanxin_id']));
    if(!$driver_item){
    ...
    $send_data = array(
        'driver_id'=>(int)$driver_item['user_id'],
        'huanxin_id'=>$driver_item['huanxin_id'],
        'avatar'=>$driver_item['avatar'],
        'nickname'=>$driver_item['nickname']
    );
    json_send($send_data);
}