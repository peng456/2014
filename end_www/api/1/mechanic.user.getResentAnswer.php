...
if($data['type'] == 1){
    //未处理的快捷通话请求
    $question_sql = "select question.* from end_mechanic_question as question inner join  end_mechanic_quickphone_request as request using(q_id)   where request.mechanic_id  = {$mechanic_user_id} and question.type = 3 and question.q_status < 2 order by question.create_time DESC limit {$from},10";
