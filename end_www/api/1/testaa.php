...
$item_mechanic_user = 24;

//添加专注车型
if(isset($data['professional_brand'])){
    $professional_brand = explode(",", $data['professional_brand']);
    $professional_brand_sql = "insert into end_mechanic_professional_brand(mechanic_id,brand_id) values";
    $str_arraay = array();

    foreach($professional_brand as $key => $val)
    {
        $str_arraay[$key] ="(".$item_mechanic_user.",".$val.")";
    }
...
if(isset($data['professional_field'])){
    ...
    $professional_field_sql = "insert into end_mechanic_professional_field(mechanic_id,field_id) values";
    $str_arraay = array();
...
//
//
//$array = array('client_id'=>'YXA6_9J0QGPPEeSQf-lSIoaBcA','client_secret'=>'YXA6PG8db8yZbE7W4kPoX1naEnauiEs','org_name'=>'mechanic','app_name'=>'mechanic');
//
//
//$test = new Easemob($array);
//
////echo  $test->getToken();
//
//
////$result = $test->isOnline("kele");     //status
////$result = $test->batchDeleteUser();
//
//$result = $test->getUserOnline(true);
//
//var_dump($result);
//
//
