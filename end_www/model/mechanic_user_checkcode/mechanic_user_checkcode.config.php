...
$end_models['mechanic_user_checkcode'] = array(
	...
	'list_fields' => array(
		...
		'expires_in'=>array(
			...
			'width'=>'auto',
			'type'=>'text',
			'filter'=>'show_mechanic_checkcode_date'
		),
		'status'=>array(
			'name'=>'状态',
			'width'=>'auto',
		...
		'_options'=>array(//显示操作里面的按钮
			'name'=>'操作',
			'width'=>100,
			'filter'=>'show_mechanic_checkcode_options'
		)
	)
);

function show_mechanic_checkcode_date($t)
{
...
//添加权限设置项
$end_rights[] = array(
	'name'=>'mechanic_user_checkcode',
	'description'=>'验证码数据',
	'rights'=>array('view','delete','update','add')
);

function show_mechanic_checkcode_options($item)
{
	$id = 'checkcode_id';
